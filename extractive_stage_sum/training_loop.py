...
def train(model, loss, optimizer, train_dataloader, val_dataloader, epochs, device,
          ...
          accumulation_steps=1, use_mixed_precision=False):
    ...
    for epoch_num in range(starting_epoch, epochs+1):
        ...
        if (epoch_num % checkpoints_frequency == 0) or is_best or (epoch_num == epochs):
            ckpt_path = os.path.join(checkpoints_path, f"cp_{model.model_name.replace('/', '_')}_ep{epoch_num:03d}.tar")
            _createCheckpoint(ckpt_path, epoch_num, model, optimizer, val_metrics.averages())
            if is_best:
